...
def motMetricsEnhancedCalculator(ground_truth_source, test_source):
    ...
    for frame in range(int(ground_truth[:, 1].max())):
        ...
        if frame == 646:
            print("debug")
        ground_truth_detections = ground_truth[ground_truth[:, 1] == frame, 1:6]  # select all detections in gt
        test_detections = test[test[:, 1] == frame, 1:6]  # select all detections in t

        C = mm.distances.iou_matrix(ground_truth_detections[:, 1:], test_detections[:, 1:], \
                                    max_iou=0.5)  # format: gt, t

        # Call update once for per frame.
        # format: gt object ids, t object ids, distance
        acc.update(ground_truth_detections[:, 0].astype('int').tolist(), \
                   test_detections[:, 0].astype('int').tolist(), C)
