...
class InputOutputHandler:
    ...
    def handle_output(self, detector):
        if self.frame_no % 20 == 0:
            print(f"Processed {'{:.1f}'.format(self.frame_no / self.frames_total * 100)} % of video. Runtimes [ms]: Enhance: {detector.enhance_time_ms} | DetectTrack: {detector.detection_tracking_time_ms} | Total: {detector.total_runtime_ms} | FPS: {int(1000 / detector.total_runtime_ms)}")
