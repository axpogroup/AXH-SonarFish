...
class Object:
    ...
    def draw_bounding_box(self, img, color):
        ...
        cv.putText(img, (self.classification[-1] + " " + str(self.ID)),
    ...
    def draw_classifications_box(self, img):
        ...
        cv.rectangle(img, (x, y), (x + w, y + h), color, 2)
        cv.putText(img, (self.classification[-1]),
                         (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)
...
class FishDetector:
    def __init__(self, filename):
        ...
        self.mean_stddev_file = fname_temp[0] + "/mean_std_dev/" + fname_temp[1] + "_mean_stddev_.npz"
        try:
    ...
    def enhance_frame(self):
        ...
        self.current_enhanced = self.spatial_filter(self.current_enhanced, kernel_size=15, method='median')
    ...
    def draw_associations(self, img, color):
        for association in self.associations:
            cv.line(img, self.detections[association["detection_id"]].midpoints[-1],
                    self.current_objects[association["existing_object_id"]].midpoints[-1], color, 2)
            cv.putText(img, str(association["distance"]), self.detections[association["detection_id"]].midpoints[-1],
    ...
    def filter_objects(self):
        ...
        for ID, obj in self.current_objects.items():
            ...
            if obj.occurences_in_last_x(self.frame_number, self.min_occurences_in_last_x_frames[1]) \
    ...
    def associate_detections(self):
        ...
        object_midpoints = [existing_object.midpoints[-1] for _, existing_object in self.current_objects.items()]
        ...
        for detection_id, detection in self.detections.items():
            ...
            if min_dist < self.max_association_dist:
                self.associations.append({"detection_id": detection.ID,
    ...
    def find_points_of_interest(self, img, mode='contour'):
        if mode == 'contour':
            ...
            contours, hier = cv.findContours(thres, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
            ...
            im_with_keypoints = cv.drawKeypoints(img, keypoints, img, (0, 0, 255),
...
if __name__ == "__main__":
    ...
    while video_cap.isOpened():
        ...
        if enhanced:
            ...
            output, raw_classified = detector.process_frame(enhanced_frame, raw_frame[1080:, :, :])
        else:
        ...
        cv.putText(disp, "FPS : " + str(int(fps)), (150, 50), cv.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
