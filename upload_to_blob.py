...
recording_files_mac = "/Users/leivandresen/Documents/PROJECTS/SONAR_FISH/Field_test_Stroppel_20to24_10_22/" \
                      "weekend_backup/"
...
snippet_path = os.path.join(folder_for_snippets, os.path.splittext(os.path.split(recordings[-2])[-1])[0]) + \
               "_snippet.mp4"
snippet_cmd = f"ffmpeg -i {recordings[-2]} -c:v libx264 -preset medium -crf 46 {snippet_path}"
success = False
try:
    output = subprocess.run(snippet_cmd, check=True, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                            text=True)
    print(output.stdout)
